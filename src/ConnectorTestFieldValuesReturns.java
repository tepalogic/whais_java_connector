...
import net.whais.Client.FieldValue;
...
public class ConnectorTestFieldValuesReturns {
    ...
    public ConnectorTestFieldValuesReturns() throws ConnException
    {
    ...
    main (String[] args) throws IOException
    {
        ...
        ConnectorTestFieldValuesReturns test =
                new ConnectorTestFieldValuesReturns ();


        boolean testResult = true;

      testResult = testResult && test.testTables (c, 0);
      testResult = testResult && test.testTables (c, 1);
      testResult = testResult && test.testTables (c, 10);
      testResult = testResult && test.testTables (c, 64);
      testResult = testResult && test.testTables (c, 128);
      testResult = testResult && test.testTables (c, 1024);
    ...
    boolean
    testTables (Connection c, int rows) throws IOException
    {
        System.out.println ("Testing "
                + "field values returns with " + rows + " row(s) ...");

        for ( int i = 0;  i < this.tableFields.length; ++i)
        {
            System.out.println ("\tTesting field '"
                                 + this.tableFields[i].getName () + "'.");

            TableValue refTable = this.getOneFieldTable (i);

            this.fillTable (refTable, rows);

            c.pushStackValue (refTable);
            c.pushStackValue (
                      Value.createBasic (ValueType.textType (),
                                         this.tableFields[i].getName ())
                             );
            ...
            {
                Value fv = result.get (r);
                Value tv = refTable.get (this.tableFields[i].getName (), r);
