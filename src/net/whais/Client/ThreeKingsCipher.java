...
final class ThreeKingsCipher implements Cipher {
    ...
    encodeFrame (CommunicationFrame frame, byte[] key)
    {
        ...
        buffer.putShort (_c.FRAME_HDR_SIZE + _c.ENC_3K_SPARE_OFF, (short)0xFFFF);

        encodeBuffer (buffer,
                       _c.FRAME_HDR_SIZE + _c.ENC_3K_PLAIN_SIZE_OFF,
                       bufferSize,
                       key,
                       firstKing,
                       secondKing);
    ...
    decodeFrame (CommunicationFrame frame, byte[] key)
    {
        ...
        final int firstKing = buffer.getInt (_c.FRAME_HDR_SIZE
                                             + _c.ENC_3K_FIRST_KING_OFF);
        final int secondKing = buffer.getInt (_c.FRAME_HDR_SIZE
                                              + _c.ENC_3K_SECOND_KING_OFF);
        decodeBuffer (buffer,
                      _c.FRAME_HDR_SIZE + _c.ENC_3K_PLAIN_SIZE_OFF,
                      bufferSize,
                      key,
    ...
    static void
    encodeBuffer (ByteBuffer    buffer,
                  int           from,
                  int           to,
                  byte[]        key,
                  int           firstKing,
                  int           secondKing)
    {
        ...
        for (int pos = from; pos < to; pos += 4)
        {
            ...
            int thirdKing = key[keyIndex++] & 0xFF; thirdKing <<= 8;
            keyIndex %= key.length;

            thirdKing |= key[keyIndex++] & 0xFF; thirdKing <<= 8;
            keyIndex %= key.length;
    ...
    static int
    exchange_1bit_pair (int value, int p1, int p2)
    {
      int val1 = (value >> p1) & 1;
      int val2 = (value >> p2) & 1;
    ...
    static int
    exchange_2bit_pair (int value, int p1, int p2)
    {
    ...
    static int
    exchange_4bit_pair (int value, int p1, int p2)
    {
    ...
    static int
    exchange_8bit_pair (int value, int p1, int p2)
    {
    ...
    static int
    exchange_16bit_pair (int value, int p1, int p2)
    {
