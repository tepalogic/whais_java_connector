...
import net.whais.Client.CommunicationFrame;
...
public class Connection {
	...
	{	
	    ...
	    this.userId = userId;
	    this.frame = new CommunicationFrame (s, userId, database, key);
	}
	...
	public void close (boolean discardCmds)
	{
	    ...
	    try {
	        if (discardCmds)
	            frame.discardPendingCommand ();
	        else
	            frame.flushPendingCommand ();
	        
	        frame.sendCommand(_c.CMD_CLOSE_CONN, false);
	    } catch (IOException e) {
	...
	public String[] retrieveGlobalNames () throws IOException 
	{
	    ...
	    else if (frame.hasPendingCommands ())
	        throw new ConnException (CmdResult.INCOMPLETE_CMD);
	...
	public ProcedureParameters describeProcedure (String name) throws IOException
	{
        ...
        do
        {
            ...
            if (ValueType.isTable (ptype))
            {
                ...
                FieldValueType[] fields = null;
                
                if (fieldsCount > 0)
                {
                    fields = new FieldValueType[fieldsCount];

                    for (int i = 0; i < fieldsCount; ++i)
                    {
                        int startOff = b.position ();
                        while (b.get() != 0)
                                ; //Let the nuffer's position advance
                        int endOff = b.position ();
                        
                        final short fieldType = b.getShort ();
                        fields[i] = new FieldValueType (new String (b.array (),
                                                                    startOff,
                                                                    endOff - startOff,
                                                                    StandardCharsets.UTF_8),
                ...
                paramTypes[param] = ValueType.create (ptype, fields);
            }
            else
	...
	public void pushStackValue (ValueType type) throws ConnException
	{
	    ...
	    {
	        for (FieldValueType f : type.getFields ())
	        {
	            ...
	            buffer.put ((byte) 0);
	            buffer.putShort ((short)f.getType ());
	        }
	    }
	    this.frame.markBufferPositionValid ();
	...
	public void updateStackTop (Value val) throws ConnException
	{
	    ...
	    else if (val.type ().isTable ())
	    {
	        ...
	        {
	            for (FieldValueType fields : table.getFields() )
	            {
   	...
   	private final void updateStackTopBasic (Value value) throws ConnException
	{
	    ...
	    buffer.put ((byte) _c.CMD_UPDATE_FUNC_CHTOP);
	    buffer.putShort ((short) value.type ().getType ());
	    buffer.put (v);
	    buffer.put((byte) 0);
	...
	private final void updateStackTopBasic (Value value, String fieldName, long row) throws ConnException
	{
       ...
       buffer.put ((byte) _c.CMD_UPDATE_FUNC_CHTOP);
	   buffer.putShort ((short) (value.type ().getType () & ValueType.FIELD_MASK));
	   buffer.put (f);
	   buffer.put((byte) 0);
	   buffer.putLong (row);
	   buffer.put (v);
	...
	private final void updateStackTopBasicArray (Value[] values, long arrayOffset) throws ConnException
	{
	    ...
	    for (Value value : values)
	    {
            ...
            if (count++ == 0)
            {
                ...
                buffer.putShort ((short) (value.type ().getType () & ValueType.ARRAY_MASK));
	...
	private final void updateStackTopText (String value, long textOffset) throws ConnException
	{
        ...
        if (buffer.position () + 1 + 2 + 8 + 4 + 1 > buffer.capacity ())
        {
            ...
            // + 1 -> the ending 0
            assert frame.hasPendingCommands ();
            this.frame.flushPendingCommand ();
    ...
    private final ValueType internalDescribeValue (String name) throws IOException
    {
       ...
       do
       {
               ...
               || (buffer.position () >= this.frame.getCmdLastPosition ()))
           {
              ...
              if ((type != buffer.getShort ()) 
                  || (fieldsCount != ((int) buffer.getShort () & 0x0000FFFF))
                  || (field != ((int) buffer.getShort () & 0x0000FFFF)))
               {
                   throw new ConnException (CmdResult.GENERAL_ERR, "Unexpected answer format received from server.");
               }
           }
           ...
           fields[field] = new FieldValueType (new String (buffer.array (), 
                                                            startOffset, 
                                                            endOffset - startOffset,
                                                            StandardCharsets.UTF_8),
       ...
       return ValueType.create (type, fields);
    }
